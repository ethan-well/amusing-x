...
import (
	...
	"amusingx.fit/amusingx/services/webApi/app/subproductapp"
	"context"
...
func getAndValidParamsV2(r *http.Request) (*proto.SubProductPicturesRequest, aerror.Error) {
	...
	if subProductIdsStr == "" {
		return nil, aerror.NewError(nil, aerror.Code.CParamsErr, "params 'sub_product_ids' is nil")
	}
	...
	for _, idStr := range subProductIdsStrArr {
		...
		if err != nil {
			return nil, aerror.NewError(err, aerror.Code.CParamsErr, "params 'sub_product_ids' is invalid")
		}
		ids = append(ids, id)
	}
	if len(ids) == 0 {
		return nil, aerror.NewError(nil, aerror.Code.CParamsErr, "params 'sub_product_ids' is invalid")
	}
