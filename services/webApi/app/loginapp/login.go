...
type Domain struct {
	LoginInfo     *apiService.LoginRequest
...
// 设置登录信息
func (d *Domain) SetLoginRequestInfo(loginRequest *apiService.LoginRequest) aerror.Error {
	if loginRequest == nil {
...
func (d *Domain) ValidVerificationCode(ctx context.Context) aerror.Error {
	...
	if !codeStore.Check(d.LoginInfo.VerificationCode) {
		return aerror.NewError(nil, aerror.Code.CParamsErr, "验证码错误")
	}
